...
plugin.checkError = function(e, resp, expectedStatus) {
  if (!e && resp && resp.statusCode !== expectedStatus) {
    ...
    if (code === 401) {
      ...
      // A forbidden request is not proof of an expired login. Preserve diagnostics
      // without logging cookies, request headers, source code, or response bodies.
      const headers = resp.headers || {};
      const responseType = String(headers['content-type'] || 'unknown').replace(/[\r\n]/g, ' ');
      const challenge = headers['cf-mitigated'] === 'challenge' ? ' The response requests a browser challenge.' : '';
      e = {
        msg: 'LeetCode rejected the request (HTTP 403 Forbidden; response type: ' + responseType + ').' +
             challenge + ' No judge result was returned; this does not establish an expired session.',
        statusCode: code
      };
    } else {
...
plugin.getProblem = function(problem, needTranslation, cb) {
  ...
  request.post(opts, function(e, resp, body) {
    ...
    problem.testcase = q.exampleTestcaseList.join("\n");
...
function formatResult(result) {
  const x = {
    ...
    runtime_percentile: typeof result.runtime_percentile === 'number' ? result.runtime_percentile : '',
    memory:             result.status_memory || '',
    memory_percentile:  typeof result.memory_percentile === 'number' ? result.memory_percentile : '',
  ...
  // Preserve the judge's verdict; displayed answers may have valid alternatives.
  x.compare_result = result.compare_result;
  x.correct_answer = null;
  if (typeof result.correct_answer === 'boolean') {
    x.correct_answer = result.correct_answer;
  } else if (typeof result.compare_result === 'string' && /^[01]+$/.test(result.compare_result)) {
    x.correct_answer = !result.compare_result.includes('0');
  } else if (Number.isInteger(result.total_correct) && Number.isInteger(result.total_testcases) &&
             result.total_testcases > 0 && result.total_correct >= 0 &&
             result.total_correct <= result.total_testcases) {
    x.correct_answer = result.total_correct === result.total_testcases;
  }
  ...
  if (/[runcode|interpret].*/.test(result.submission_id)) {
    // It's testing
    // Keep case boundaries before the legacy console formatter joins the output.
    x.stdout_by_case = Array.isArray(result.std_output_list) ? result.std_output_list :
      Array.isArray(result.code_output) ? result.code_output : undefined;
