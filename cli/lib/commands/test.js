...
const cmd = {
  ...
  builder: function(yargs) {
    return yargs
      .option('webview', { type: 'boolean', default: false, describe: 'Include structured results for VS Code' })
      .option('i', {
        alias:    'interactive',
        type:     'boolean',
        default:  false,
...
function runTest(argv) {
  ...
  core.getProblem(meta.id, true, function(e, problem) {
    ...
    if (argv.testcase)
      problem.testcase = argv.i ? argv.testcase : argv.testcase.replace(/\\n/g, '\n');
    ...
    core.testProblem(problem, function(e, results) {
      ...
      if ((actual.state === 'Accepted' || actual.state === 'Finished') && actual.error.length === 0) {
        if (typeof actual.correct_answer === 'boolean') {
          actual.state = actual.correct_answer ? 'Accepted' : 'Wrong Answer';
          actual.ok = actual.correct_answer;
        } else {
      ...
      if (argv.webview) {
        const payload = {
          version: 1,
          status: actual.state,
          runtime: String(actual.runtime || ''),
          input: problem.testcase,
          metadata: problem.templateMeta || {},
          outputs: Array.isArray(actual.answer) ? actual.answer : [],
          expected: Array.isArray(actual.expected_answer) ? actual.expected_answer : [],
          comparison: typeof actual.compare_result === 'string' ? actual.compare_result : '',
          correct: typeof actual.correct_answer === 'boolean' ? actual.correct_answer : null,
          errors: actual.error,
          stdoutByCase: actual.stdout_by_case,
          stdout: actual.stdout || ''
        };
        // Encode to keep multiline output and user text separate from CLI log records.
